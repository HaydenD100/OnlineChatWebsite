...
const requestListener = function (req, res) {
    ...
    if(req.url.substr(0,5) == "/send")
    {
        ...
        fs.appendFile("chats.txt", "\n" + name + ":"+ decodeURI(req.url.substr(5)), (err) => { 
            if (err) 
              console.log(err); 
        ...
        res.end("<html><body><pre>" + data +"\n" + name + ":"+  decodeURI(req.url.substr(5)) +"<pre></body></html>");
        
    });

    }
    else if(req.url.substr(0,5) == "/name"){
        if(req.url.substr(5) == "") name = "anonymous"
        else name = req.url.substr(5);
